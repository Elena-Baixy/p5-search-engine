...
def load_index():
    ...
    if not os.path.exists("index_server/index/stopwords.txt"):
        shutil.copyfile(
            "inverted_index/stopwords.txt", "index_server/index/stopwords.txt"
        )
...
def query_cleaning(query):
    ...
    stopwords_list = []
    filtered_query = {}
    with open("inverted_index/stopwords.txt",
              "r", encoding="utf-8") as stopwords:
...
def get_doc_hits():
    ...
    d_vector_list, idf_list, d_norm_list = doc_vector(
        filtered_query, output_doc, term_tf
    )
    ...
    tf_idf_s = tf_idf_score(d_vector_list, normalized_q_vector,
                            q_vector, d_norm_list)
...
def doc_vector(filtered_query, output_doc, term_tf):
    """Calculate doc vector. doc_list should be doc."""
    ...
    for doc in output_doc:
        for term, _ in filtered_query.items():
            with open(file_to_find,
                      "r", encoding="utf-8") as inverted_index_file:
                for line in inverted_index_file:
                    term_read = line.split(" ")[0]
                    if term == term_read:
                        idf = line.split()[1]
...
def tf_idf_score(d_vector_list, normalized_q_vector, q_vector, d_norm_list):
    ...
    for doc, _ in d_vector_list.items():
        ...
        score = score / (
            float(normalized_q_vector) * math.sqrt(float(d_norm_list[doc]))
        )
        tf_idf_s[doc] = score
    # print(tf_idf_s)
    return tf_idf_s
...
def final_result(weighted_s):
    """I am a doc."""
    result = []
    sorted_weighted_s = dict(sorted(weighted_s.items(),
